...
* Comment choisir les extensions que vous souhaitez.
* Quelles extensions Active Support fournit.
...
Cet exemple montre comment charger [`Hash#with_indifferent_access`][Hash#with_indifferent_access]. Cette extension permet la conversion d'un `Hash` en [`ActiveSupport::HashWithIndifferentAccess`][ActiveSupport::HashWithIndifferentAccess] qui permet d'accéder aux clés sous forme de chaînes de caractères ou de symboles.
...
INFO : Le prédicat pour les chaînes de caractères utilise la classe de caractères sensible à l'Unicode `[:space:]`, donc par exemple U+2029 (séparateur de paragraphe) est considéré comme un espace.
...
Lorsque vous souhaitez appeler une méthode sur un objet uniquement s'il n'est pas `nil`, la manière la plus simple de le faire est d'utiliser des instructions conditionnelles, ce qui ajoute un encombrement inutile. L'alternative est d'utiliser [`try`][Object#try]. `try` est comme `Object#public_send`, sauf qu'il renvoie `nil` s'il est envoyé à `nil`.
Voici un exemple :
...
Notez que `try` va ignorer les erreurs de méthode inexistante, renvoyant à la place `nil`. Si vous voulez vous protéger contre les fautes de frappe, utilisez [`try!`][Object#try!] à la place :
...
Par exemple, `nil`, `true` et `false` renvoient eux-mêmes. [`Array#to_param`][Array#to_param] appelle `to_param` sur les éléments et joint le résultat avec "/":
...
La méthode [`to_query`][Object#to_query] construit une chaîne de requête qui associe une clé donnée à la valeur renvoyée par `to_param`. Par exemple, avec la définition `to_param` suivante :
...
La méthode [`instance_values`][Object#instance_values] renvoie un hachage qui fait correspondre les noms de variables d'instance sans "@" à leurs valeurs correspondantes. Les clés sont des chaînes de caractères :
...
### Suppression des avertissements et des exceptions
...
AVERTISSEMENT: Notez que dans ce cas, `module_parent_name` renvoie `nil`.
...
La méthode [`module_parents`][Module#module_parents] appelle `module_parent` sur le receveur et remonte jusqu'à ce que `Object` soit atteint. La chaîne est renvoyée dans un tableau, du bas vers le haut:
...
La macro [`delegate`][Module#delegate] offre un moyen facile de déléguer des méthodes.
...
Imaginez que vous souhaitez déléguer tout ce qui manque à l'objet `User` à l'objet `Profile`. La macro [`delegate_missing_to`][Module#delegate_missing_to] vous permet de mettre en œuvre cela facilement:
...
La méthode [`class_attribute`][Class#class_attribute] déclare un ou plusieurs attributs de classe héritables qui peuvent être remplacés à n'importe quel niveau de la hiérarchie.
...
Pour plus de commodité, `class_attribute` définit également un prédicat d'instance qui est la double négation de ce que renvoie le lecteur d'instance. Dans les exemples ci-dessus, il serait appelé `x?`.
Lorsque `:instance_reader` est `false`, le prédicat d'instance renvoie une `NoMethodError` tout comme la méthode de lecture.
...
#### Chaînes de caractères sécurisées

Active Support a le concept de chaînes de caractères _(html) sécurisées_. Une chaîne de caractères sécurisée est marquée comme pouvant être insérée dans du HTML telle quelle. Elle est considérée comme fiable, que ce soit échappée ou non.

Par défaut, les chaînes de caractères sont considérées comme _non sécurisées_ :

```ruby
"".html_safe? # => false
```

Vous pouvez obtenir une chaîne de caractères sécurisée à partir d'une chaîne donnée avec la méthode [`html_safe`][String#html_safe] :
...
La méthode [`squish`][String#squish] supprime les espaces vides en début et en fin de chaîne, et remplace les séries d'espaces vides par un seul espace :
...
La méthode [`truncate`][String#truncate] renvoie une copie de la chaîne de caractères tronquée après une longueur donnée :
...
La méthode [`truncate_bytes`][String#truncate_bytes] renvoie une copie de son objet tronqué à un maximum de `bytesize` octets :
...
La méthode [`truncate_words`][String#truncate_words] renvoie une copie de son objet tronqué après un certain nombre de mots :
...
La méthode [`inquiry`][String#inquiry] convertit une chaîne de caractères en un objet `StringInquirer`, rendant les comparaisons d'égalité plus jolies.
...
cette quantité d'espaces en début de ligne.
...
La méthode [`indent`][String#indent] indente les lignes de l'objet :
...
La méthode [`at`][String#at] renvoie le caractère de la chaîne de caractères à la position `position` :
...
La méthode [`from`][String#from] renvoie la sous-chaîne de caractères de la chaîne de caractères à partir de la position `position` :
...
La méthode [`to`][String#to] renvoie la sous-chaîne de caractères de la chaîne de caractères jusqu'à la position `position` :
...
La méthode [`first`][String#first] renvoie une sous-chaîne de caractères contenant les `limit` premiers caractères de la chaîne de caractères.
...
La méthode [`last`][String#last] renvoie une sous-chaîne de caractères contenant les `limit` derniers caractères de la chaîne de caractères.
...
La méthode [`pluralize`][String#pluralize] renvoie le pluriel de son objet :
...
La méthode [`camelize`][String#camelize] renvoie son objet en camel case :
...
La méthode [`underscore`][String#underscore] fait l'inverse, du camel case aux chemins:
...
La méthode [`titleize`][String#titleize] met en majuscule les mots dans la chaîne:
...
La méthode [`dasherize`][String#dasherize] remplace les underscores dans la chaîne par des tirets:
...
La méthode [`parameterize`][String#parameterize] normalise la chaîne de manière à pouvoir être utilisée dans les URL conviviales.
...
La méthode [`constantize`][String#constantize] résout l'expression de référence constante dans son receveur :
...
La méthode [`humanize`][String#humanize] ajuste un nom d'attribut pour l'affichage aux utilisateurs finaux.
...
La méthode [`foreign_key`][String#foreign_key] donne le nom de la colonne de clé étrangère à partir d'un nom de classe. Pour ce faire, elle démodule, ajoute des tirets bas et ajoute "_id" :
...
Ces méthodes peuvent être combinées avec [`from_now`][Duration#from_now], [`ago`][Duration#ago], etc, pour des calculs de date précis. Par exemple :
...
La méthode [`ordinal`][Integer#ordinal] renvoie la chaîne de suffixe ordinal correspondant à l'entier:
...
La méthode [`ordinalize`][Integer#ordinalize] renvoie la chaîne ordinale correspondant à l'entier. En comparaison, notez que la méthode `ordinal` renvoie **seulement** la chaîne de suffixe.
...
La méthode [`index_by`][Enumerable#index_by] génère un hash avec les éléments d'une énumération indexés par une clé.
...
La méthode [`index_with`][Enumerable#index_with] génère un hash avec les éléments d'une énumération comme clés. La valeur
...
Le prédicat [`exclude?`][Enumerable#exclude?] teste si un objet donné n'appartient **pas** à la collection. C'est la négation de `include?` intégré:
...
La méthode [`including`][Enumerable#including] renvoie une nouvelle énumération qui inclut les éléments passés en argument:
...
La méthode [`excluding`][Enumerable#excluding] renvoie une copie d'une énumération avec les éléments spécifiés
supprimés:
...
Active Support améliore l'API des tableaux pour faciliter certaines façons de les accéder. Par exemple, [`to`][Array#to] renvoie le sous-tableau des éléments jusqu'à celui à l'index passé :
...
De même, [`from`][Array#from] renvoie la partie à partir de l'élément à l'index passé jusqu'à la fin. Si l'index est supérieur à la longueur du tableau, il renvoie un tableau vide.
...
La méthode [`including`][Array#including] renvoie un nouveau tableau qui inclut les éléments passés :
...
La méthode [`extract!`][Array#extract!] supprime et renvoie les éléments pour lesquels le bloc renvoie une valeur vraie.
...
Dans ces cas, vous pouvez donner à un hash d'options un traitement distinct avec [`extract_options!`][Array#extract_options!]. Cette méthode vérifie le type du dernier élément d'un tableau. S'il s'agit d'un hash, il le supprime et le renvoie, sinon il renvoie un hash vide.
...
La méthode [`to_xml`][Array#to_xml] renvoie une chaîne de caractères contenant une représentation XML de son objet :
...
La méthode [`Array.wrap`][Array.wrap] enveloppe son argument dans un tableau à moins qu'il ne soit déjà un tableau (ou de type tableau).
...
#### `in_groups_of(nombre, remplir_avec = nil)`
...
#### `in_groups(nombre, remplir_avec = nil)`
...
# </hash>
```
Pour ce faire, la méthode boucle sur les paires et construit des nœuds qui dépendent des _valeurs_. Étant donné une paire `clé`, `valeur` :
...
Et une version avec un point d'exclamation [`reverse_merge!`][Hash#reverse_merge!] qui effectue la fusion sur place :
...
Active Support définit [`Hash#deep_merge`][Hash#deep_merge]. Dans une fusion profonde, si une clé est présente dans les deux hashes et que leurs valeurs sont elles-mêmes des hashes, alors leur _fusion_ devient la valeur dans le hash résultant :
...
La méthode [`Hash#deep_dup`][Hash#deep_dup] se duplique elle-même ainsi que toutes les clés et valeurs à l'intérieur de manière récursive avec la méthode Active Support `Object#deep_dup`. Elle fonctionne comme `Enumerator#each_with_object` en envoyant la méthode `deep_dup` à chaque paire à l'intérieur.
...
La méthode [`except`][Hash#except] renvoie un hash avec les clés de la liste d'arguments supprimées, si elles sont présentes :
...
Il existe également la variante [`except!`][Hash#except!] qui supprime les clés sur place.
...
La méthode [`stringify_keys`][Hash#stringify_keys] renvoie un hash dont les clés du receveur sont converties en chaînes de caractères. Elle le fait en envoyant `to_s` à chaque clé :
...
Il existe également la variante [`stringify_keys!`][Hash#stringify_keys!] qui convertit les clés en chaînes de caractères sur place.

En plus de cela, on peut utiliser [`deep_stringify_keys`][Hash#deep_stringify_keys] et [`deep_stringify_keys!`][Hash#deep_stringify_keys!] pour convertir en chaînes de caractères toutes les clés du hash donné et de tous les hashes imbriqués. Un exemple du résultat est :
...
La méthode [`symbolize_keys`][Hash#symbolize_keys] renvoie un hash dont les clés du receveur sont symbolisées, dans la mesure du possible. Elle le fait en envoyant `to_sym` à chaque clé :
...
Il existe également la variante avec point d'exclamation [`symbolize_keys!`][Hash#symbolize_keys!] qui symbolise les clés sur place.

En plus de cela, on peut utiliser [`deep_symbolize_keys`][Hash#deep_symbolize_keys] et [`deep_symbolize_keys!`][Hash#deep_symbolize_keys!] pour symboliser toutes les clés du hash donné et tous les hashes imbriqués. Un exemple du résultat est :
...
La méthode [`deep_transform_values`][Hash#deep_transform_values] renvoie un nouveau hash avec toutes les valeurs converties par l'opération de bloc. Cela inclut les valeurs du hash racine et de tous les hashes et tableaux imbriqués.
...
Il existe également la variante avec point d'exclamation [`deep_transform_values!`][Hash#deep_transform_values!] qui convertit de manière destructive toutes les valeurs en utilisant l'opération de bloc.
...
La méthode [`slice!`][Hash#slice!] remplace le hash par les seules clés données et renvoie un hash contenant les paires clé/valeur supprimées.
...
La méthode [`multiline?`][Regexp#multiline?] indique si une expression régulière a le drapeau `/m` défini, c'est-à-dire si le point correspond aux sauts de ligne.
...
INFO: `beginning_of_hour`, `end_of_hour`, `beginning_of_minute` et `end_of_minute` sont implémentés pour `Time` et `DateTime` mais **pas** pour `Date` car il n'a pas de sens de demander le début ou la fin d'une heure ou d'une minute sur une instance de `Date`.
...
La méthode [`ago`][Date#ago] reçoit un nombre de secondes en argument et renvoie un horodatage correspondant à il y a autant de secondes depuis minuit :
...
De même, [`since`][Date#since] avance dans le temps :
...
AVERTISSEMENT : `DateTime` ne tient pas compte des règles relatives à l'heure d'été, de sorte que certaines de ces méthodes ont des cas particuliers lorsqu'un changement d'heure d'été est en cours. Par exemple, [`seconds_since_midnight`][DateTime#seconds_since_midnight] peut ne pas renvoyer la véritable quantité dans un tel jour.
...
Cette méthode est également un alias de [`getutc`][DateTime#getutc].
...
La méthode [`change`][DateTime#change] vous permet d'obtenir un nouveau datetime qui est identique au récepteur à l'exception des options données, qui peuvent inclure `:year`, `:month`, `:day`, `:hour`, `:min`, `:sec`, `:offset`, `:start` :
...
[`prev_day`][Time#prev_day] et [`next_day`][Time#next_day] renvoient le moment du jour précédent ou suivant :
...
[`prev_quarter`][DateAndTime::Calculations#prev_quarter] et [`next_quarter`][DateAndTime::Calculations#next_quarter] renvoient la date avec le même jour du trimestre précédent ou suivant:
...
`prev_quarter` est un alias de [`last_quarter`][DateAndTime::Calculations#last_quarter].
...
Active Support définit [`Time.current`][Time.current] comme `Time.zone.now` s'il existe un fuseau horaire utilisateur défini, avec une fallback à `Time.now`:
...
Les objets [`Duration`][ActiveSupport::Duration] peuvent être ajoutés et soustraits des objets temps:
...
NOTE. En raison de l'opération chmod que `atomic_write` effectue, si le fichier cible a un ACL défini dessus, cet ACL sera recalculé/modifié.
